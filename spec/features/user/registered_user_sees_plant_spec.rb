describe 'as a registered user' do
  it 'creates a plant happy path' do
    ...
    plant_2 = garden.plants.create(name: 'Tom', species: 'Carrot', frequency: 12, last_watered: Time.now)

    allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(user_1)
    # As registered user.
    visit plants_path(plant_1.id)
    ...
    expect(page).to have_content("Last Watered: #{plant_1.last_watered}")
    expect(page).to_not have_content("Name: #{plant_2.name}")
    expect(page).to_not have_content("Last Watered: #{plant_2.last_watered}")
